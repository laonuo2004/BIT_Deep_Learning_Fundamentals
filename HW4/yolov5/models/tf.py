...
class TFBN(keras.layers.Layer):
    ...
    def __init__(self, w=None):
        ...
        super().__init__()
        # 使用 PyTorch 模型的 BatchNorm 层权重初始化 TensorFlow 的 BatchNorm 层
...
class TFConv(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, w=None):
        ...
        # TensorFlow 卷积层，根据步长设置填充方式
        conv = keras.layers.Conv2D(
            filters=c2,
            kernel_size=k,
            strides=s,
            padding="SAME" if s == 1 else "VALID",
            use_bias=not hasattr(w, "bn"),  # 如果有 BatchNorm 则不使用偏置
            kernel_initializer=keras.initializers.Constant(w.conv.weight.permute(2, 3, 1, 0).numpy()),  # 权重初始化
            bias_initializer="zeros" if hasattr(w, "bn") else keras.initializers.Constant(w.conv.bias.numpy()),  # 偏置初始化
        )
        # 如果步长 s != 1，则在卷积前添加填充层
        self.conv = conv if s == 1 else keras.Sequential([TFPad(autopad(k, p)), conv])
        # 批归一化层，如果 PyTorch 模型有 BN 层则使用 TFBN，否则为恒等映射
...
class TFDWConvTranspose2d(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        # 将输入按通道分割，分别通过对应的 Conv2DTranspose 层，然后拼接并裁剪边缘
...
class TFFocus(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        """
        # 将输入在空间维度上进行切片，然后拼接在通道维度上，实现下采样和通道扩展
...
class TFBottleneck(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        result.
        """
        # 前向传播：如果使用残差连接，则将输入 inputs 与卷积结果相加
...
class TFBottleneckCSP(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        dimensions.
        """
        # CSP 结构：一部分通过 TFBottleneck 模块，另一部分直接通过卷积
...
class TFC3(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        """
        # C3 结构：一部分通过 TFBottleneck 模块，另一部分直接通过卷积，然后拼接并通过卷积层
...
class TFC3x(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        # C3x 结构：一部分通过 TFCrossConv 模块，另一部分直接通过卷积，然后拼接并通过卷积层
        return self.cv3(tf.concat((self.m(self.cv1(inputs)), self.cv2(inputs)), axis=3))
...
class TFDetect(keras.layers.Layer):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), imgsz=(640, 640), w=None):
        ...
        """
        super().__init__()
        self.stride = tf.convert_to_tensor(w.stride.numpy(), dtype=tf.float32)  # 步长
        self.nc = nc  # 类别数
        self.no = nc + 5  # 每个锚点输出的数量 (x, y, w, h, conf, class_probs)
        self.nl = len(anchors)  # 检测层数量
        self.na = len(anchors[0]) // 2  # 锚点数量
        self.grid = [tf.zeros(1)] * self.nl  # 网格
        self.anchors = tf.convert_to_tensor(w.anchors.numpy(), dtype=tf.float32)  # 锚点
        # 锚点网格，用于计算预测框的绝对坐标
    ...
    def call(self, inputs):
        ...
        for i in range(self.nl):
            ...
            ny, nx = self.imgsz[0] // self.stride[i], self.imgsz[1] // self.stride[i]
            # 重塑输出张量为 (batch_size, grid_cells, anchors, outputs_per_anchor)
...
class TFSegment(TFDetect):
    ...
    def call(self, x):
        ...
        # 如果是训练模式，返回检测结果和原型掩码；否则只返回检测结果和原型掩码
...
def parse_model(d, ch, model, imgsz):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # 将 PyTorch 模块名转换为对应的 TensorFlow 模块名
        tf_m = eval("TF" + m_str.replace("nn.", ""))
        # 创建 TensorFlow 模块实例，并加载 PyTorch 模型的权重
        m_ = (
...
def activations(act=nn.SiLU):
    ...
    # 将 PyTorch 激活函数转换为 TensorFlow 对应的激活函数
