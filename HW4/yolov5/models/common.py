...
def autopad(k, p=None, d=1):
    ...
    """
    # 如果膨胀率 d > 1，则计算实际的卷积核大小
    if d > 1:
        k = d * (k - 1) + 1 if isinstance(k, int) else [d * (x - 1) + 1 for x in k]  # actual kernel-size
    # 如果未指定填充 p，则自动计算填充大小以实现“same”填充
    if p is None:
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        # 激活函数，默认为 SiLU，也可指定其他激活函数或不使用激活函数
    ...
    def forward_fuse(self, x):
        ...
        # 融合前向传播：卷积 -> 激活 (用于推理时，将BN层参数融合到卷积层中)
...
class DWConv(Conv):
    ...
    def __init__(self, c1, c2, k=1, s=1, d=1, act=True):
        ...
        """
        # 深度可分离卷积，通过设置 groups=math.gcd(c1, c2) 实现
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers):
        ...
        super().__init__()
        self.conv = None
        # 如果输入通道 c1 不等于输出通道 c2，则添加一个卷积层进行通道调整
        if c1 != c2:
            self.conv = Conv(c1, c2)
        # 可学习的位置编码
        self.linear = nn.Linear(c2, c2)  # learnable position embedding
        # 堆叠多个 TransformerLayer
    ...
    def forward(self, x):
        ...
        if self.conv is not None:
            x = self.conv(x)
        b, _, w, h = x.shape
        # 将特征图展平并调整维度以适应 Transformer 输入 (seq_len, batch_size, embed_dim)
        p = x.flatten(2).permute(2, 0, 1)
        # Transformer 层处理，并添加位置编码，然后恢复特征图形状
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        ...
        # 是否使用残差连接，当 shortcut 为 True 且输入输出通道相同时启用
...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        enhanced output.
        """
        # CSP 结构：一部分通过 Bottleneck 模块，另一部分直接通过卷积
        y1 = self.cv3(self.m(self.cv1(x)))
        y2 = self.cv2(x)
        # 将两部分结果拼接，然后通过批归一化、激活和卷积层
...
class C3(nn.Module):
    ...
    def forward(self, x):
        ...
        # C3 结构：一部分通过 Bottleneck 模块，另一部分直接通过卷积，然后拼接并通过卷积层
...
class C3TR(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        # 将 Bottleneck 替换为 TransformerBlock
...
class C3Ghost(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        # 将 Bottleneck 替换为 GhostBottleneck
...
class SPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress torch 1.9.0 max_pool2d() warning
            # 连续三次最大池化，然后将原始特征与三次池化结果拼接，再通过卷积层
            y1 = self.m(x)
            y2 = self.m(y1)
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):
        ...
        super().__init__()
        c_ = c2 // 2
        # 卷积分支：GhostConv (pw) -> DWConv (dw) -> GhostConv (pw-linear)
        self.conv = nn.Sequential(
            GhostConv(c1, c_, 1, 1),  # pw
            DWConv(c_, c_, k, s, act=False) if s == 2 else nn.Identity(),  # dw
            GhostConv(c_, c2, 1, 1, act=False),
        )  # pw-linear
        # 残差连接分支：如果步长 s=2，则进行 DWConv 和 Conv，否则为恒等映射
        self.shortcut = (
...
class Contract(nn.Module):
    ...
    def forward(self, x):
        ...
        s = self.gain
        # 将空间维度 (h, w) 缩小 s 倍，并将缩小的部分移动到通道维度
